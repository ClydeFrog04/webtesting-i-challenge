...
describe("Gets item", () => {
    it("Should get an item with the name set to [+7] Iron Sword", () => {
        const item = {enhancement: 7, durability: 100, name: "Iron Sword"};
        const newItem = enhancer.get(item);
        expect(newItem.name).toBe("[+7] Iron Sword");
    });

    it("Should get an item with the name set to Iron Sword", () => {
        const item = {enhancement: 0, durability: 100, name: "Iron Sword"};
        const newItem = enhancer.get(item);
        expect(newItem.name).toBe("Iron Sword");
    });
});